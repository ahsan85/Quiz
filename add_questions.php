...
{ 
  ...
  if (!empty($_POST['ques'])  &&  !empty($_POST['answers']) && !empty($_POST['correct']) && !empty($_POST['opType']) ) {
     // var_dump($_POST);
   //   die();
      $q = mysqli_real_escape_string($connection, $_POST['ques']);
      $_opType = mysqli_real_escape_string($connection, $_POST['opType']);
      $_correct = mysqli_real_escape_string($connection, $_POST['correct']);
     ...
     $sql_insert_question="INSERT  INTO questions (questions,type,answer_id) VALUES ('$q', '$_opType',null)";
        ...
        if(mysqli_query($connection,$sql_insert_question)){
          ...
          foreach ($_answers as $key => $value) {
            ...
            $answers = mysqli_real_escape_string($connection, $value[0]);
...
<body>
     <div class="container">
     	...
     	<form class="form" method="post">
               ...
               <label>
                    Option 3
                  <input type="checkbox" name="correct[option3]">
                </label>
